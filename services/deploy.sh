...
if [[ $(hostname) == "toolbox" ]]; then
  ...
  sops_cmd="/usr/bin/flatpak-spawn --host docker run --interactive --rm --security-opt label=disable --volume ${PWD}:/pwd --workdir /pwd --volume ${HOME}/.config/sops/age/keys.txt:/pwd/keys.txt:ro -e SOPS_AGE_KEY_FILE=/pwd/keys.txt nixery.dev/sops sops"
else
  docker_cmd="docker"
  docker_compose_cmd="docker-compose"
  sops_cmd="docker run --interactive --rm --security-opt label=disable --volume ${PWD}:/pwd --workdir /pwd --volume ${HOME}/.config/sops/age/keys.txt:/pwd/keys.txt:ro -e SOPS_AGE_KEY_FILE=/pwd/keys.txt nixery.dev/sops sops"
fi
